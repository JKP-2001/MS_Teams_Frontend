...
export default function AddNote(props) {
    ...
    const {addNote} = useContext(GrpContext);
    ...
    const handleAdd = async ()=>{
        setLoading(true);
        const result = await addNote(title,desc);
        setLoading(false);
        

        if(!result.success){
            showToast({
                msg:"Error in adding note",
                type:"error",
                duration:3000
            })
            setShowModal(false);
                                ...
                                <div className="relative p-4 w-full max-w-2xl h-full md:h-auto">
                                    ...
                                    <div className="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                        ...
                                        <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                                            ...
                                            <button type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="updateProductModal" onClick={() => {setShowModal(false); setDesc(""); setTitle("")}}>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center space-x-4">
                                                <button  className="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" onClick={handleAdd}>
                                                    {loading?"Adding ...":"Add note"}
                                                </button>
                                                <button type="button" className="text-red-600 inline-flex items-center hover:text-white border border-red-600 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900" onClick={() => {setShowModal(false); setDesc(""); setTitle("")}}>
                                                    Cancel
                                                </button>
                                            </div>
                                        </div>
